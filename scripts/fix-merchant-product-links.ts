/**
 * Fix Merchant Center product links → https://thekashmirweaver.com/products/{handle}
 *
...
const HYDROGEN_ROOT = resolve(ROOT, "../hydrogen-the-kashmir-weaver");
...
function googleToken() {
  return execFileSync(
    "gcloud",
    ["auth", "print-access-token", "--scopes=https://www.googleapis.com/auth/content"],
    { encoding: "utf8" },
  ).trim();
}
...
async function main() {
  ...
  console.log(
    `Wrong-domain links: ${
      products.filter((p) => p.link && !p.link.includes("thekashmirweaver.com")).length
    }`,
  );
  ...
  async function worker() {
    while (queue.length) {
      ...
      if (p.link === link && p.canonicalLink === canonicalLink) {
        skipped++;
        continue;
      }
      ...
      if (p.googleProductCategory) productAttributes.googleProductCategory = p.googleProductCategory;
      if (p.itemGroupId) productAttributes.itemGroupId = p.itemGroupId;
      productAttributes.ageGroup = "ADULT";
      productAttributes.gender = "UNISEX";
      if (p.price) {
